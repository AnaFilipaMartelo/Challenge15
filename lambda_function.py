...
def validate_data(age, dollars, intent_request):
    ...
    # Validate that the user is between 1 and 64 years old
    if age is not None:
        age = parse_int(
            age
            )
        if not (0 < age < 65):
            return build_validation_result(
                False,
                "age",
                "You should be between 1 and 64 years old to use this service, "
                "please provide a different age.",
            )

    # Validate the investment amount, it should be >= 5000
    if dollars is not None:
        dollars = parse_float(dollars)  # Since parameters are strings it's important to cast values
        if dollars < 5000:
            return build_validation_result(
                False,
                "dollars",
...
def recommend_portfolio(intent_request):
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            "content": """Thank you, {}. Based on your inputs, I recommend the following portfolio: {}
